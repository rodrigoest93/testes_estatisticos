...
Na análise acima, podemos verificar os graus de correlação entre dos dois cruzamentos: a potência bruta (hp) possui fraca correlação negativa com o número de marchas (gear); enquanto que ao peso (wt) possui correlação forte positiva com a potência bruta. Mesmo com toda a técnica aplicada ainda existe uma etapa da análise de correlação que muitas vezes é esquecida e não menos importante: a significância dos coeficiente.
...
Tudo isso pode ser definido também, em um gráfico interessante da biblioteca do `ggplot`, onde já são feitos os devidos cáculos de correlação e a análise gráfica. De sobra, ainda nos traz o $R^2$, tendo mais um métrica para nossa análise.

```{r, echo=TRUE}

grafico_regressao <- function(df, x, y, color){
  ggplot2::ggplot(df,aes(x = {{x}}, y = {{y}}, color = {{color}})) +
    ggplot2::geom_smooth() +
      ggplot2::theme_minimal()
}
...
iris %>% 
grafico_regressao(x = Sepal.Length, y = Petal.Length, color = "red")
```
